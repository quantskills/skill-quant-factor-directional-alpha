---
name: quant-real-factor-5d-time-series-ranked-skip-period-momentum
description: Use when computing the 5D Time-Series Ranked Skip-Period Momentum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Skip-Period Momentum

Use this Skill to compute `5日时序排名跳期收益动量` / `5D Time-Series Ranked Skip-Period Momentum` from caller-provided OHLCV data.
