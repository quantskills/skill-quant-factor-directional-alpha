---
name: quant-real-factor-7d-volatility-scaled-return-momentum
description: Use when computing the 7D Volatility Scaled Return Momentum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Return Momentum

Use this Skill to compute `7日波动缩放收益动量` / `7D Volatility Scaled Return Momentum` from caller-provided OHLCV data.
