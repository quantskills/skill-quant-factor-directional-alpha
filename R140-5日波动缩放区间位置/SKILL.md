---
name: quant-real-factor-5d-volatility-scaled-range-position
description: Use when computing the 5D Volatility Scaled Range Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Range Position

Use this Skill to compute `5日波动缩放区间位置` / `5D Volatility Scaled Range Position` from caller-provided OHLCV data.
