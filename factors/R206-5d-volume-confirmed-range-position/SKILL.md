---
name: quant-real-factor-5d-volume-confirmed-range-position
description: Use when computing the 5D Volume Confirmed Range Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volume Confirmed Range Position

Use this Skill to compute `5日量能确认区间位置` / `5D Volume Confirmed Range Position` from caller-provided OHLCV data.
