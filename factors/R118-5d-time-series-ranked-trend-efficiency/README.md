...
`5日时序排名趋势效率` 是一个使用真实行情验证过的 OHLCV 因子 Skill，属于 `趋势` 类。

### 因子逻辑

```text
rank(efficiency, window=5)
```
...
`5D Time-Series Ranked Trend Efficiency` is a real-data-validated framework-neutral OHLCV factor Skill in the `Trend` family.
