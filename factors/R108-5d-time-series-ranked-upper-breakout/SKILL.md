---
name: quant-real-factor-5d-time-series-ranked-upper-breakout
description: Use when computing the 5D Time-Series Ranked Upper Breakout factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Upper Breakout

Use this Skill to compute `5日时序排名上轨突破` / `5D Time-Series Ranked Upper Breakout` from caller-provided OHLCV data.
