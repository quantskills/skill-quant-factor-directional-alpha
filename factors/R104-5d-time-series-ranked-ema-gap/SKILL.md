---
name: quant-real-factor-5d-time-series-ranked-ema-gap
description: Use when computing the 5D Time-Series Ranked EMA Gap factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked EMA Gap

Use this Skill to compute `5日时序排名EMA偏离` / `5D Time-Series Ranked EMA Gap` from caller-provided OHLCV data.
