---
name: quant-real-factor-7d-time-series-ranked-range-position
description: Use when computing the 7D Time-Series Ranked Range Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Range Position

Use this Skill to compute `7日时序排名区间位置` / `7D Time-Series Ranked Range Position` from caller-provided OHLCV data.
