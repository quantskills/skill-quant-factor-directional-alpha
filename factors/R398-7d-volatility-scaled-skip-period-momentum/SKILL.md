---
name: quant-real-factor-7d-volatility-scaled-skip-period-momentum
description: Use when computing the 7D Volatility Scaled Skip-Period Momentum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Skip-Period Momentum

Use this Skill to compute `7日波动缩放跳期收益动量` / `7D Volatility Scaled Skip-Period Momentum` from caller-provided OHLCV data.
