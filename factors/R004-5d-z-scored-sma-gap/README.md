...
`5日标准化均线偏离` 是一个使用真实行情验证过的 OHLCV 因子 Skill，属于 `趋势` 类。
...
该因子将 `均线偏离` 与 `标准化` 处理结合，观察价格、量能或波动状态在真实市场横截面中的可排序性。
...
- 可用行数: `195455`
- 覆盖率: `0.9925`
- 5日 Rank IC 均值: `-0.005959`
- 5日 ICIR: `-0.2130`
- 五分组 Q5-Q1 均值: `-0.000630`
- Top 组换手: `0.4924`
- 无未来函数检查: `True`
- 状态: `pass`
...
`5D Z-Scored SMA Gap` is a real-data-validated framework-neutral OHLCV factor Skill in the `Trend` family.
...
This factor combines `SMA Gap` with a `Z-Scored` transform to test whether the resulting price, volume, or volatility state is cross-sectionally sortable in real market data.
...
- Usable rows: `195455`
- Coverage: `0.9925`
- 5-day Rank IC mean: `-0.005959`
- 5-day ICIR: `-0.2130`
- Quintile Q5-Q1 mean: `-0.000630`
- Top-quintile turnover: `0.4924`
