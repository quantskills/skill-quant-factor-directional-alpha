---
name: quant-real-factor-5d-time-series-ranked-risk-adjusted-trend
description: Use when computing the 5D Time-Series Ranked Risk-Adjusted Trend factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Risk-Adjusted Trend

Use this Skill to compute `5日时序排名风险调整趋势` / `5D Time-Series Ranked Risk-Adjusted Trend` from caller-provided OHLCV data.
