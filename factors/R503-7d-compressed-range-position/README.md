...
`7日压缩区间位置` 是一个框架中立的 OHLCV 因子 Skill，属于 `通道` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
compress(range_position, window=7)
```
...
`7D Compressed Range Position` is a framework-neutral OHLCV factor Skill in the `Channel` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
