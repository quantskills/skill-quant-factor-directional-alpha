---
name: quant-real-factor-7d-compressed-range-position
description: Use when computing the 7D Compressed Range Position factor from user-supplied OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Compressed Range Position

Use this Skill to compute `7日压缩区间位置` / `7D Compressed Range Position` from caller-provided OHLCV data.
