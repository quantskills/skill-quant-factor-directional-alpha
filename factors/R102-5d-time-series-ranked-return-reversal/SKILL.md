---
name: quant-real-factor-5d-time-series-ranked-return-reversal
description: Use when computing the 5D Time-Series Ranked Return Reversal factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Return Reversal

Use this Skill to compute `5日时序排名收益反转` / `5D Time-Series Ranked Return Reversal` from caller-provided OHLCV data.
