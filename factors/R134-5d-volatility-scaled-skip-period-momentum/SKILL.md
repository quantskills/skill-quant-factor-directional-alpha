---
name: quant-real-factor-5d-volatility-scaled-skip-period-momentum
description: Use when computing the 5D Volatility Scaled Skip-Period Momentum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Skip-Period Momentum

Use this Skill to compute `5日波动缩放跳期收益动量` / `5D Volatility Scaled Skip-Period Momentum` from caller-provided OHLCV data.
