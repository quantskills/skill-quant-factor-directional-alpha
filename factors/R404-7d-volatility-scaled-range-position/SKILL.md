---
name: quant-real-factor-7d-volatility-scaled-range-position
description: Use when computing the 7D Volatility Scaled Range Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Range Position

Use this Skill to compute `7日波动缩放区间位置` / `7D Volatility Scaled Range Position` from caller-provided OHLCV data.
