---
name: quant-real-factor-7d-time-series-ranked-sma-slope
description: Use when computing the 7D Time-Series Ranked SMA Slope factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked SMA Slope

Use this Skill to compute `7日时序排名均线斜率` / `7D Time-Series Ranked SMA Slope` from caller-provided OHLCV data.
