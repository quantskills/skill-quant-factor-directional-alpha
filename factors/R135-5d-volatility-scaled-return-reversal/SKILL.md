---
name: quant-real-factor-5d-volatility-scaled-return-reversal
description: Use when computing the 5D Volatility Scaled Return Reversal factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Return Reversal

Use this Skill to compute `5日波动缩放收益反转` / `5D Volatility Scaled Return Reversal` from caller-provided OHLCV data.
