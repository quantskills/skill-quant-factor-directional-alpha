---
name: quant-real-factor-5d-z-scored-range-position
description: Use when computing the 5D Z-Scored Range Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Z-Scored Range Position

Use this Skill to compute `5日标准化区间位置` / `5D Z-Scored Range Position` from caller-provided OHLCV data.
