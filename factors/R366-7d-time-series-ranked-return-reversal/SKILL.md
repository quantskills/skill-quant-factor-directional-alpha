---
name: quant-real-factor-7d-time-series-ranked-return-reversal
description: Use when computing the 7D Time-Series Ranked Return Reversal factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Return Reversal

Use this Skill to compute `7日时序排名收益反转` / `7D Time-Series Ranked Return Reversal` from caller-provided OHLCV data.
