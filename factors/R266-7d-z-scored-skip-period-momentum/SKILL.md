---
name: quant-real-factor-7d-z-scored-skip-period-momentum
description: Use when computing the 7D Z-Scored Skip-Period Momentum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Z-Scored Skip-Period Momentum

Use this Skill to compute `7日标准化跳期收益动量` / `7D Z-Scored Skip-Period Momentum` from caller-provided OHLCV data.
