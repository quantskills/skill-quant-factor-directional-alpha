---
name: quant-real-factor-5d-time-series-ranked-sma-slope
description: Use when computing the 5D Time-Series Ranked SMA Slope factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked SMA Slope

Use this Skill to compute `5日时序排名均线斜率` / `5D Time-Series Ranked SMA Slope` from caller-provided OHLCV data.
