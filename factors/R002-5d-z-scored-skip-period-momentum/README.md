...
`5日标准化跳期收益动量` 是一个使用真实行情验证过的 OHLCV 因子 Skill，属于 `动量` 类。

### 因子逻辑

```text
z(skip_return, window=5)
```
...
`5D Z-Scored Skip-Period Momentum` is a real-data-validated framework-neutral OHLCV factor Skill in the `Momentum` family.
