---
name: quant-real-factor-5d-z-scored-skip-period-momentum
description: Use when computing the 5D Z-Scored Skip-Period Momentum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Z-Scored Skip-Period Momentum

Use this Skill to compute `5日标准化跳期收益动量` / `5D Z-Scored Skip-Period Momentum` from caller-provided OHLCV data.
