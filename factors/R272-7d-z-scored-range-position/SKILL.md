---
name: quant-real-factor-7d-z-scored-range-position
description: Use when computing the 7D Z-Scored Range Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
Use this Skill to compute `7日标准化区间位置` / `7D Z-Scored Range Position` from caller-provided OHLCV data.
