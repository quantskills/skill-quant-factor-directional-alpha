---
name: quant-real-factor-5d-compressed-return-momentum
description: Use when computing the 5D Compressed Return Momentum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Compressed Return Momentum

Use this Skill to compute `5日压缩收益动量` / `5D Compressed Return Momentum` from caller-provided OHLCV data.
