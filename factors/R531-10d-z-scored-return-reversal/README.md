...
`10日标准化收益反转` 是一个框架中立的 OHLCV 因子 Skill，属于 `反转` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
z(reversal, window=10)
```
...
- 市场覆盖: `{"cn": 98, "us": 50}`
- 覆盖率: `0.9857`
- 5日 Rank IC 均值: `-0.008727`
- 5日 ICIR: `-0.3271`
- 五分组 Q5-Q1 均值: `-0.001963`
- Top 组换手: `0.3314`
- 无未来函数检查: `True`
- 状态: `pass`
...
`10D Z-Scored Return Reversal` is a framework-neutral OHLCV factor Skill in the `Reversal` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9857`
- 5-day Rank IC mean: `-0.008727`
- 5-day ICIR: `-0.3271`
- Quintile Q5-Q1 mean: `-0.001963`
- Top-quintile turnover: `0.3314`
