---
name: quant-real-factor-7d-stability-scaled-range-position
description: Use when computing the 7D Stability Scaled Range Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Stability Scaled Range Position

Use this Skill to compute `7日稳定度缩放区间位置` / `7D Stability Scaled Range Position` from caller-provided OHLCV data.
