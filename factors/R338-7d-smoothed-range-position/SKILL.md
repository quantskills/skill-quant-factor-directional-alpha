---
name: quant-real-factor-7d-smoothed-range-position
description: Use when computing the 7D Smoothed Range Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Smoothed Range Position

Use this Skill to compute `7日平滑区间位置` / `7D Smoothed Range Position` from caller-provided OHLCV data.
