---
name: quant-real-factor-7d-volume-confirmed-range-position
description: Use when computing the 7D Volume Confirmed Range Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volume Confirmed Range Position

Use this Skill to compute `7日量能确认区间位置` / `7D Volume Confirmed Range Position` from caller-provided OHLCV data.
