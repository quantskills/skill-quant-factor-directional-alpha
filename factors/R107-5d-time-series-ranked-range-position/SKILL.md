---
name: quant-real-factor-5d-time-series-ranked-range-position
description: Use when computing the 5D Time-Series Ranked Range Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Range Position

Use this Skill to compute `5日时序排名区间位置` / `5D Time-Series Ranked Range Position` from caller-provided OHLCV data.
