---
name: quant-real-factor-5d-time-series-ranked-lower-breakdown
description: Use when computing the 5D Time-Series Ranked Lower Breakdown factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Lower Breakdown

Use this Skill to compute `5日时序排名下轨跌破` / `5D Time-Series Ranked Lower Breakdown` from caller-provided OHLCV data.
