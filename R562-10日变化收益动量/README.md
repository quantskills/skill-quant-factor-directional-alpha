...
`10日变化收益动量` 是一个框架中立的 OHLCV 因子 Skill，属于 `动量` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
delta(return, window=10)
```

该因子把“收益动量”与“变化”处理结合，用来刻画价格、量能或波动状态在横截面上的可排序性。
...
`10D Delta Return Momentum` is a framework-neutral OHLCV factor Skill in the `Momentum` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
This factor combines Return Momentum with a Delta transform to test cross-sectional sortability in price, volume, or volatility states.
...
- 5-day ICIR: `-0.2165`
- Quintile Q5-Q1 mean: `-0.000159`
- Top-quintile turnover: `0.5588`
