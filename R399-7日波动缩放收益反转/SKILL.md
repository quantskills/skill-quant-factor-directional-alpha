---
name: quant-real-factor-7d-volatility-scaled-return-reversal
description: Use when computing the 7D Volatility Scaled Return Reversal factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Return Reversal

Use this Skill to compute `7日波动缩放收益反转` / `7D Volatility Scaled Return Reversal` from caller-provided OHLCV data.
