...
`10日压缩上轨突破` 是一个框架中立的 OHLCV 因子 Skill，属于 `突破` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
compress(breakout, window=10)
```

该因子把“上轨突破”与“压缩”处理结合，用来刻画价格、量能或波动状态在横截面上的可排序性。
...
`10D Compressed Upper Breakout` is a framework-neutral OHLCV factor Skill in the `Breakout` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
This factor combines Upper Breakout with a Compressed transform to test cross-sectional sortability in price, volume, or volatility states.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9925`
- 5-day Rank IC mean: `0.028023`
- 5-day ICIR: `0.9246`
- Quintile Q5-Q1 mean: `0.001636`
- Top-quintile turnover: `0.4204`
