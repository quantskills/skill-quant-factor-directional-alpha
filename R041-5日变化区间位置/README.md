...
`5日变化区间位置` 是一个使用真实行情验证过的 OHLCV 因子 Skill，属于 `通道` 类。

### 因子逻辑

```text
delta(range_position, window=5)
```

该因子将 `区间位置` 与 `变化` 处理结合，观察价格、量能或波动状态在真实市场横截面中的可排序性。
...
`5D Delta Range Position` is a real-data-validated framework-neutral OHLCV factor Skill in the `Channel` family.
...
This factor combines `Range Position` with a `Delta` transform to test whether the resulting price, volume, or volatility state is cross-sectionally sortable in real market data.
