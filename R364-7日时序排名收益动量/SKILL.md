---
name: quant-real-factor-7d-time-series-ranked-return-momentum
description: Use when computing the 7D Time-Series Ranked Return Momentum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Return Momentum

Use this Skill to compute `7日时序排名收益动量` / `7D Time-Series Ranked Return Momentum` from caller-provided OHLCV data.
