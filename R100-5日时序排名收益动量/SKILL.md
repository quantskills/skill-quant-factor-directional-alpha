---
name: quant-real-factor-5d-time-series-ranked-return-momentum
description: Use when computing the 5D Time-Series Ranked Return Momentum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Return Momentum

Use this Skill to compute `5日时序排名收益动量` / `5D Time-Series Ranked Return Momentum` from caller-provided OHLCV data.
