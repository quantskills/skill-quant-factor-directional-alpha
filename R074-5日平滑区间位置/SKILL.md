---
name: quant-real-factor-5d-smoothed-range-position
description: Use when computing the 5D Smoothed Range Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Smoothed Range Position

Use this Skill to compute `5日平滑区间位置` / `5D Smoothed Range Position` from caller-provided OHLCV data.
